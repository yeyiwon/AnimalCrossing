    ...
    const fetchData = async () => {
        ...
        try{
            ...
            const Recipes = recipes.map(recipe => {
                const krKoName = recipe.translations.kRko;
                const recipesData = response.data.find(data => data.name === recipe.name)

                return{
                    ...recipesData, krKoName};
            });
        ...
        <div className='container'>
            ...
            <ul className='itemcard'>
                {searchedData.map((item) => ( 
                    <Link to={`/RecipesList/${item.id}`} key={item.id}>
                        <li>
